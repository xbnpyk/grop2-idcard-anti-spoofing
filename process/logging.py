...
class Logger_tensorboard:
    ...
    #     # log losses
    #     total_loss = sum(losses.values())
    #     Logger_tensorboard._sw.add_scalar(f'Eval/loss_total', total_loss, eval_step)
    #     for n, l in losses.items():
    #         Logger_tensorboard._sw.add_scalar(f'Eval/loss_{n}', l, eval_step) 
    ...
    #     Logger_tensorboard._sw.add_figure(f"Predicted/generated", Logger_tensorboard._plot_spectrogram(predicted_spec), eval_step)
    #     Logger_tensorboard._sw.add_figure(f"Predicted/forced", Logger_tensorboard._plot_spectrogram(f_predicted_spec), eval_step)
    #     Logger_tensorboard._sw.add_figure(f"Target/eval", Logger_tensorboard._plot_spectrogram(target_spec), eval_step) 
        
    #     # log audio
    #     waveform = audio.inverse_spectrogram(predicted_spec, not hp.predict_linear)
    #     Logger_tensorboard._sw.add_audio(f"Audio/generated", waveform, eval_step, sample_rate=hp.sample_rate)  
    #     waveform = audio.inverse_spectrogram(f_predicted_spec, not hp.predict_linear)
    #     Logger_tensorboard._sw.add_audio(f"Audio/forced", waveform, eval_step, sample_rate=hp.sample_rate)              
        
    #     # log alignment
    #     alignment = alignment[idx, :target_len[idx], :source_len[idx]].data.cpu().numpy().T
    #     Logger_tensorboard._sw.add_figure(f"Alignment/eval", Logger_tensorboard._plot_alignment(alignment), eval_step)                
        
    #     # log source text
    #     utterance = text.to_text(source[idx].data.cpu().numpy()[:source_len[idx]], hp.use_phonemes)
    #     Logger_tensorboard._sw.add_text(f"Text/eval", utterance, eval_step)      
        
    #     # log stop tokens
    #     Logger_tensorboard._sw.add_figure(f"Stop/eval", Logger_tensorboard._plot_stop_tokens(stop_target[idx].data.cpu().numpy(), stop_prediction[idx].data.cpu().numpy()), eval_step) 
        
    #     # log mel cepstral distorsion
    #     Logger_tensorboard._sw.add_scalar(f'Eval/mcd', mcd, eval_step)
        
    #     # log reversal language classifier accuracy
    #     if hp.reversal_classifier:
    #         Logger_tensorboard._sw.add_scalar(f'Eval/classifier', classifier, eval_step)
